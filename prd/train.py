...
def main():
    ...
    default_model_name = f"{args.env}_{args.algo}_recurrence{args.recurrence}_seed{args.seed}_reshape_{args.reshape_reward}_{timestamp}"
    ...
    for i in range(args.procs):
        envs.append(utils.make_env(args.env, args.seed + 10000 * i, args.reshape_reward, horizon = None))
    ...
    while num_frames < args.frames:
        ...
        if update % args.log_interval == 0:
            ...
            stats_per_episode = {}
            for i in range(envs[0].stat.shape[0]):
                stats_per_episode[str(i)] = utils.synthesize(envs[0].stat[i])["mean"]
            ...
            header += ["hole_" + i for i in stats_per_episode.keys()]

            data += stats_per_episode.values()

            txt_logger.info(
                "U {} | F {:06} | FPS {:04.0f} | D {} | rR:μσmM {:.2f} {:.2f} {:.2f} {:.2f} | F:μσmM {:.1f} {:.1f} {} {} | H {:.3f} | V {:.3f} | pL {:.3f} | vL {:.3f} | ∇ {:.3f} | R:μσmM {:.2f} {:.2f} {:.2f} {:.2f}| H: {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f}"
                .format(*data))
        ...
        if data[-4] > best_data[-4]:
            ...
            txt_logger.info("Best Status saved")
